...
def show_index():
    ...
    :return:
    """
    print(f"欢迎{core.get_online().get_username()}来到个人博客首页")
    print("~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~！~*~")
    print("1.修改密码")
    print("2.维护个人资料")
    print("3.文章管理")
    ...
    if choice == "1":
        """修改密码"""
        res = core.get_online().user_change_password()
        if res == "ok":
            core.dump_data()
            print("修改成功！")
            return show_login()
...
def show_user_information():
    ...
    res = core.get_online().user_perfect_information()
    if res == "error":
        print("退出资料维护")
        time.sleep(1)
        return show_index()
