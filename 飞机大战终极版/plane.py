...
class Resource:
    ...
    # 英雄拾到补给后的护盾组
    group_supply_age = pygame.sprite.Group()
    # 英雄飞机图片
    hero_images = [
    ...
    # 爆炸效果图片
    bomb_images = [
        "images/bomp/enemy2_down1.png",
        "images/bomp/enemy2_down2.png",
        "images/bomp/enemy2_down3.png",
        "images/bomp/enemy2_down4.png",
        "images/bomp/enemy2_down5.png",
        "images/bomp/enemy2_down6.png"
    ]
    # 系统不同关卡背景速度
...
class HeroSprite(GameSprite):
    ...
    def event(self):
        """事件控制"""
        # 鼠标控制
        is_in = self.rect.collidepoint(pygame.mouse.get_pos())
        if is_in and HeroSprite.is_click:
...
class SupplySprite(pygame.sprite.Sprite):
    """补给精灵"""
...
class Engine:
    ...
    def __init__(self):
        ...
        self.step = 1
        # 创建敌机定时器
        pygame.time.set_timer(Resource.enemy_event_create, random.randint(1000/self.step, 2000/self.step))
        # 创建补给定时类
        pygame.time.set_timer(Resource.supply_event, random.randint(4000/self.step, 8000/self.step))
        # 关卡分数设置
    ...
    def all_event(self):
        ...
        if len(event_list) > 0:
            for event in event_list:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        HeroSprite.is_click = True
                        if StartSprite.is_in == 1:
                            Resource.is_play = True

                if event.type == pygame.MOUSEBUTTONUP:
                    HeroSprite.is_click = False
                ...
                if event.type == Resource.supply_event:
                    supp = SupplySprite("images/supply/supply1.png", random.randint(0, Resource.screen_rect.width-150),
                                        -150, 5)
                    Resource.group_supply.add(supp)
    ...
    def update(self):
        ...
        Resource.group_supply.update()
        Resource.group_supply.draw(self.screen)
        Resource.group_supply_age.update()
        Resource.group_supply_age.draw(self.screen)
    ...
    def collide(self):
        ...
        for enemy in Resource.group_enemy:
            ...
            if len(res.keys()) > 0:
                enemy.hp -= 1
                if enemy.hp <= 0:
                    self.bomb_x = enemy.rect.x
                    self.bomb_y = enemy.rect.y
        ...
        if len(a.keys()) > 0:
            ...
            if self.hp == 0:
                self.dead_frame()
        for supp in Resource.group_supply:
            b = pygame.sprite.groupcollide(Resource.group_supply, Resource.group_hero, False, False)
            if len(b.values()) > 0:
                if len(Resource.group_supply_age) < 1:
                    print(type(supp))
                    Resource.group_supply_age.add(supp)
                    Resource.group_supply.remove(supp)

        for supp in Resource.group_supply_age:
            for hero in Resource.group_hero:
                supp.rect.center = hero.rect.center
        pygame.sprite.groupcollide(Resource.group_supply_age, Resource.group_enemy, True, True)
        pygame.sprite.groupcollide(Resource.group_supply_age, Resource.group_enemy_bullet, True, True)
    ...
    def dead_frame(self):
        ...
        while True:
            ...
            if Resource.is_play:
                ...
                Resource.group_resource.empty()
                Resource.group_jump.empty()
                Resource.group_supply_age.empty()
                return self.run()
            pygame.display.update()
    ...
    def bomb(self):
        """爆炸效果"""
        self.screen.blit(pygame.image.load(Resource.bomb_images[self.bomb_index]), (self.bomb_x, self.bomb_y))
        pygame.display.update()
