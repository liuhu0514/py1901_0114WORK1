        ...
        for key, value in i.items():
            if key == online["admin"]:
                for x, y in value.items():
                    print("发送人：{}\t内容：{}".format(x, y))
    # 展示所用用户
    print("所用用户如下：")
    for admin in users.keys():
        if admin != online["admin"]:
            print(admin, end=";")
...
def fa_new():
    ...
    if admin.lower() == "r":
        ...
        if admin != online["admin"]:
            new = input("请输入消息：")
            news.append({admin: {online["admin"]: new}})
            print("消息发送成功！")
            save_data()
            return show_index()

        else:
            print("不能给自己发消息，请重新选择发送人！")
            return fa_new()
...
def save_data():
    ...
    file["users"] = users
    file["all_articles"] = all_articles
    file["recycle_bin_dict"] = recycle_bin_dict
    file["news"] = news
...
def get_data():
    ...
    global users, all_articles, recycle_bin_dict, news
    file = shelve.open("./data/1.dat")
    users = file['users']
    all_articles = file["all_articles"]
    recycle_bin_dict = file["recycle_bin_dict"]
    news = file["news"]
